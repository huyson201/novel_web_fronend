...
import { MdOutlineBookmarkBorder, MdOutlineBookmarkAdded } from 'react-icons/md'
...
const ToolBar = ({ prevLink, nextLink, onClickBookmark, onClickListButton, bookMark = "NoAdded" }: Props) => {
    ...
    return (
        <div className={cx("toolbar", { [`show-settings`]: showSettings }, `theme-${settings?.theme ?? 'light'}`)} ref={toolbarRef}>
            <div className={cx('toolbar-controls')}>
                ...
                <button className={cx('toolbar__btn')} onClick={handleAddBookcase}>{bookMark === 'NoAdded' ? <MdOutlineBookmarkBorder className='toolbar__btn-ions' /> : <MdOutlineBookmarkAdded className='toolbar__btn-ions' />}</button>
